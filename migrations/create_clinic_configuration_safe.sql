...
-- Drop existing trigger and function if they exist (to allow re-running)
DROP TRIGGER IF EXISTS trigger_update_clinic_configuration_updated_at ON clinic_configuration;
DROP FUNCTION IF EXISTS update_clinic_configuration_updated_at();
...
DROP POLICY IF EXISTS "Anyone can read clinic configuration" ON clinic_configuration;
DROP POLICY IF EXISTS "Authenticated users can update clinic configuration" ON clinic_configuration;
