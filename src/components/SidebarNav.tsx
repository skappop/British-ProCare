      ...
      {NAV.map(({ href, label, icon: Icon }) => {
        ...
        return (
          <Link
            key={href}
            href={href}
            className={`relative flex items-center gap-3 px-3 py-2.5 rounded-control transition-all duration-200 ${
              active
          ...
          >
            {active && (
              <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[2.5px] rounded-full bg-gradient-to-b from-gold-light to-gold-deep" />
            )}
